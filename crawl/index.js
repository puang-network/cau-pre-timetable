...
async function getCurrentSemester() {
  const res = await fetch(
    "https://mportal.cau.ac.kr/std/usk/sUskSif001/selectCurYear.ajax",
    {
      method: "POST",
      headers: {
...
exports.handler = async () => {
  ...
  for (const ext of ["json", "csv"]) {
    ...
    await spawnCommand("node", [
      "node_modules/cau.ac.kr/dist/crawller",
      "--outFile",
      "/tmp/courses.tmp." + ext,
      "--type",
      ext,
  ...
  const possibleTargets = [
    path.resolve(__dirname, "..", "frontend", "public", "courses"),
    path.resolve(__dirname, "..", "public", "courses"),
  ];
  ...
  for (const targetDir of possibleTargets) {
    try {
      ...
      for (const ext of ["csv", "json"]) {
        ...
        const dest = path.join(targetDir, "courses." + ext);
        console.log(`Copying ${source} -> ${dest}`);
        await fs.copyFile(source, dest);
      }

      console.log(`Writing crawlInfo.json locally to ${targetDir}`);
      await fs.writeFile(path.join(targetDir, "crawlInfo.json"), crawlInfo, {
        encoding: "utf8",
      });
    } catch (err) {
      console.warn(`Could not write to ${targetDir}: ${err.message}`);
    }
  }
