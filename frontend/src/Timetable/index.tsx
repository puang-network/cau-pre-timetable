...
function svgTextWithLineBreak(
  ...
  return texts.map((i) => (
    <tspan fontSize={`${fontSize}px`} x={x} dy="1.2em">
      {i}
    </tspan>
  ));
}
  ...
  for (let i = 0; i < classes.length; i++) {
    ...
    for (const schedule of cls.schedules) {
      for (const time of schedule.times) {
        ...
        const tspan = fitTspansIntoHeight(
          svgTextWithLineBreak(
            `${cls.name}\n${cls.professor}`,
            x + 65,
            12,
            63,
        ...
        yield (
          <g x={x} y={y}>
            ...
            ></rect>
            <text
              fill={blackForeColors[i] ? "black" : "white"}
              textAnchor="end"
              x={x + 60}
              y={y + 5}
              fontSize="14px"
              dominantBaseline="hagging"
            >
              {tspan}
            </text>
          </g>
...
const timetable = forwardRef(function (
  opts: { classes: Class[] },
  ...
  return (
    <svg
      ref={ref}
      width={(days.length + 1) * 70}
      viewBox={`0 0 ${(days.length + 1) * 70} 950`}
    >
      <g id="days">
        <rect
          fill="whitesmoke"
